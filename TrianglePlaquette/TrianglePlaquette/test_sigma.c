...
#define CALL_AND_CHECK_STATUS(function, error_message) do { \
	...
	CALL_AND_CHECK_STATUS(mkl_sparse_z_export_csr(I, &indexing, &n_rowsI, &n_colsI, &pointerB_I, &pointerE_I, &columns_I, &valuesI),
		"Error after MKL_SPARSE_Z_EXPORT_CSR  I \n");
	CALL_AND_CHECK_STATUS(mkl_sparse_z_export_csr(X, &indexing, &n_rowsX, &n_colsX, &pointerB_X, &pointerE_X, &columns_X, &valuesX),
		"Error after MKL_SPARSE_Z_EXPORT_CSR  X \n");
	CALL_AND_CHECK_STATUS(mkl_sparse_z_export_csr(Y, &indexing, &n_rowsY, &n_colsY, &pointerB_Y, &pointerE_Y, &columns_Y, &valuesY),
		"Error after MKL_SPARSE_Z_EXPORT_CSR  Y \n");
	CALL_AND_CHECK_STATUS(mkl_sparse_z_export_csr(Z, &indexing, &n_rowsZ, &n_colsZ, &pointerB_Z, &pointerE_Z, &columns_Z, &valuesZ),
		"Error after MKL_SPARSE_Z_EXPORT_CSR  Z \n");
