...
#define CALL_AND_CHECK_STATUS(function, error_message) do { \
	...
	// test Pauli operator
	/*
	double* coefList = malloc(4 * sizeof(double));
	int** listOfPauliList = malloc(4*sizeof(int*));
	for (i = 0; i < 4; ++i) {
		coefList[i] = 1.0;
		listOfPauliList[i] = calloc(20, sizeof(int));
	}
	*/
	//CALL_AND_CHECK_STATUS(pauli_hamiltonian_matrix(&H, 20, 4, listOfPauliList, coefList), "Error during computing a triangle plaquette matrix");
